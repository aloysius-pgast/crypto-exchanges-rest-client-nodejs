...
/*
This example show how to retrieve tickers for a list of pairs
*/
...
const opt = [
    ...
    {exchange:'bittrex',pairs:['USDT-NEO','USDT-BTC']}
]
...
restClient.ping().then((running) => {
    ...
    return restClient.getServices().then((services) => {
        ...
        _.forEach(opt, (e) => {
            ...
            if (!restClient.checkExchange(services, e.exchange, ['tickers']))
            {
                arr.push(Promise.resolve(null));
                return;
            }
            arr.push(restClient.getTickers(e.exchange, e.pairs));
        });
        return Promise.all(arr).then((results) => {
            _.forEach(results, (data, index) => {
                ...
                console.log(`Tickers for '${opt[index].exchange}' :`);
