...
/*
This example show how to retrieve full history for a given pair (might not be possible on all exchanges)
*/
...
restClient.ping().then((running) => {
    ...
    return restClient.getServices().then((services) => {
        ...
        _.forEach(opt, (e) => {
            ...
            if (!restClient.checkExchange(services, e.exchange, ['closedOrders']))
            {
                arr.push(Promise.resolve(null));
                return;
            }
            arr.push(restClient.getClosedOrders(e.exchange, {pairs:e.pairs,completeHistory:true}));
        });
        return Promise.all(arr).then((results) => {
            _.forEach(results, (data, index) => {
                ...
                console.log(`Found ${Object.keys(data).length} closed orders for '${exchange}' (will only display 3 randomly) :`);
