...
/*
This example show how to retrieve last trades
*/
...
restClient.ping().then((running) => {
    ...
    return restClient.getServices().then((services) => {
        ...
        _.forEach(opt, (e) => {
            ...
            if (!restClient.checkExchange(services, e.exchange, ['trades']))
            {
                arr.push(Promise.resolve(null));
                return;
            }
            arr.push(restClient.getTrades(e.exchange, e.pair));
        });
        return Promise.all(arr).then((results) => {
            _.forEach(results, (data, index) => {
                ...
                console.log(`Trades for '${opt[index].pair}' on '${opt[index].exchange}' (will only display a sample) :`);
                let result =  Helpers.describeArray(data);
